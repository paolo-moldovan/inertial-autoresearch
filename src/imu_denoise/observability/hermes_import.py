"""Read-only import of Hermes session state into mission-control storage."""
...
def _transcript_blob(writer: ObservabilityWriter, messages: list[dict[str, Any]]) -> str | None:
    return writer.store_json_blob(messages)
...
def _import_session(
    *,
    ...
    writer.append_event(
        ...
        event_type=HERMES_SESSION_IMPORTED,
        level="INFO",
        ...
        fingerprint=ObservabilityWriter._fingerprint(origin, session_id, "session"),
    )
    ...
    writer.append_event(
        run_id=run_id,
        session_id=session_id,
        event_type=HERMES_TRANSCRIPT_IMPORTED,
        level="INFO",
        title="Hermes transcript imported",
        payload={"message_count": len(messages), "transcript_blob_ref": blob_ref},
        source=origin,
        created_at=started_at,
        fingerprint=ObservabilityWriter._fingerprint(origin, session_id, "transcript"),
    )
    ...
    if skills:
        writer.record_skill_event(
            ...
            source=origin,
            fingerprint=ObservabilityWriter._fingerprint(origin, session_id, "skills", skills),
        )
...
def _import_sqlite_state(writer: ObservabilityWriter, db_path: Path) -> int:
    cursor_real, _ = (
...
def _import_json_sessions(writer: ObservabilityWriter, sessions_dir: Path) -> int:
    cursor_real, _ = (
...
def import_hermes_state(*, writer: ObservabilityWriter, hermes_home: Path) -> dict[str, int]:
    ...
    if state_db.exists():
        counts["sqlite_sessions"] = _import_sqlite_state(writer, state_db)
    elif sessions_dir.exists():
        counts["json_sessions"] = _import_json_sessions(writer, sessions_dir)
        return counts
