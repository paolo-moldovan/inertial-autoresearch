"""Generic Hermes provider integration for bounded candidate selection."""
...
def hermes_backend_ready(config: Any, *, root: Path) -> bool:
    """Return whether Hermes and the configured local model endpoint look reachable."""
    python_bin = root / str(config.python_bin)
    cli_path = root / str(config.cli_path)
    if not python_bin.exists() or not cli_path.exists():
        return False

    for url in _healthcheck_urls(str(config.base_url)):
        try:
            with urlopen(url, timeout=float(config.healthcheck_timeout_sec)):
                return True
...
def choose_mutation_proposal_with_trace(
    *,
    ...
    root: Path,
    project_skills_dir: Path | None = None,
) -> tuple[CandidateProposal, HermesQueryTrace]:
...
def _run_hermes_query(
    *,
    ...
    if getattr(config, "toolsets", None):
        command.extend(["--toolsets", ",".join(list(config.toolsets))])
    api_key = getattr(config, "api_key", None) or _default_custom_api_key(config)
    if api_key:
        command.extend(["--api_key", api_key])

    env = os.environ.copy()
    hermes_home = (root / str(config.home_dir)).resolve()
    env["HERMES_HOME"] = str(hermes_home)
    _sync_project_skills(hermes_home, project_skills_dir)

    if getattr(config, "skills", None):
        command.extend(["--skills", ",".join(list(config.skills))])
    if bool(getattr(config, "pass_session_id", False)):
    ...
    try:
        ...
        raise HermesProposalError(
            f"Hermes timed out after {int(config.timeout_sec)} seconds.",
            trace=trace,
        ) from exc
...
def _default_custom_api_key(config: Any) -> str | None:
    if str(getattr(config, "provider", "")) != "custom":
        return None
    normalized = str(getattr(config, "base_url", "")).lower()
    if not normalized:
        return None
...
def _sync_project_skills(hermes_home: Path, project_skills_dir: Path | None) -> None:
...
def _build_prompt(
    *,
    ...
    return "\n".join(
        [
            "You are selecting the next bounded ML experiment mutation.",
            ...
            *lesson_lines,
            ...
            "- Prefer candidates that build on the current incumbent unless branching is allowed.",
            (
                "- Prefer candidates that are likely to improve the objective while "
                "keeping search diverse."
            ),
            (
...
def _healthcheck_urls(base_url: str) -> list[str]:
    ...
    if parsed.path.endswith("/v1"):
        ...
        urls.append(
            urlunparse((parsed.scheme, parsed.netloc, api_tags_path, "", "", ""))
        )
    return urls
